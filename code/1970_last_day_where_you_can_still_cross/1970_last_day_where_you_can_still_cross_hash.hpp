...
public:
    ...
    latestDayToCross(u16 row, u16 col, std::vector<std::vector<int>>& cells)
    {
        ...
        ObjectWithoutDtor<BumpVector<u16, SolutionStorage>> cell_to_group;
        cell_to_group->resize(w * h, 0xFFFF);
        ObjectWithoutDtor<
        ...
        for (auto& cell_vec : cells)
        {
            ...
            for (Vec2 offset : offsets)
            {
                ...
                if (auto g = cell_to_group.get()[neighbor_i]; g != 0xFFFF)
                {
                    neighbor_groups[num_neighbor_groups++] = g;
                }
            }
