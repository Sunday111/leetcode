...
public:
    ...
    static u32 mm(u32 a, u32 b) noexcept
    {
        return static_cast<u32>((u64{a} * b) % kMOD);
    }
    static u32& mme(u32& x, u32 m) noexcept { return x = mm(x, m); }
    ...
    u32 xorAfterQueriesImpl(
        std::vector<u32>& nums,
        std::vector<std::vector<u32>>& queries_)
    {
    ...
    int xorAfterQueries(
        std::vector<int>& nums,
        std::vector<std::vector<int>>& queries) noexcept
    {
