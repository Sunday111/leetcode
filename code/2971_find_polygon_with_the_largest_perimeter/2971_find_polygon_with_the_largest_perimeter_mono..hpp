...
public:
    ...
    using u64 = uint64_t;
    using i64 = int64_t;
    i64 largestPerimeter(std::vector<int>& nums_)
    {
        auto nums = reinterpret_range<u32>(nums_);
        radix_sort<u32, SortOrder::Ascending, 8, 4>(nums);
        u64 sum = std::accumulate(nums.begin(), nums.end(), u64{});
        const u32 n = static_cast<u32>(nums.size());
        u32 i = n - 1;

        while (i != 1 && sum <= 2 * nums[i])
        {
            sum -= nums[i--];
        }

        return i > 1 ? static_cast<i64>(sum) : -1;
    }
};
